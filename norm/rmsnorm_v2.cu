...
__global__ void RMSNorm(float *g_idata, unsigned int n) {
    ...
    int tid = threadIdx.x;
    int block_start = blockDim.x * blockIdx.x *  DATA_PER_THREAD;
    ...
    for(int i = 0; i < DATA_PER_THREAD; i++){
        r_data[i] =  g_idata[block_start+ tid* DATA_PER_THREAD + i];
    ...
    const float exp = 1e-9;
    if(tid == 0){
        s_data[0] = sqrt((s_data[0]/BLOCK_SIZE*DATA_PER_THREAD)+exp);
    }
    ...
    #pragma unroll
    for(int i =0 ; i< DATA_PER_THREAD;i++){
        g_idata[block_start+ tid* DATA_PER_THREAD + i] =  r_data[i]/s_data[0];
    }    
}
    ...
    for (int i = start; i < end; i++) {
      ...
      if (diff > epsilon) {
        if (mismatch_count < 10) { // 只打印前10个不匹配的结果
          printf(
              "Block %d, Mismatch at index %d: cpu %.6f, gpu %.6f, diff %.6e\n",
