...
__global__  void RMSNorm(float* g_idata, unsigned int n, unsigned int d){
    ...
    int tx= threadIdx.x;
    int tid = threadIdx.y * blockIdx.x + tx;

    int threads_count = blockDim.x;
    int datas_per_block = threads_count * DATA_PER_THREAD;
    int* block_begin_data_ptr = g_idata +  blockIdx.x * datas_per_block;
    int* warp_begin_data_ptr = block_begin_data_ptr + ty * warp_size* DATA_PER_THREAD;
    ...
    float reduced =  sqrtf( sm[0] / blockDim.x * DATA_PER_THREAD*  blockDim.x);
...
int main(){
    ...
    dim3 block( WARP_SIZE * DATA_PER_THREAD , block_size/(WARP_SIZE * DATA_PER_THREAD)  );
